...
def MMOE(dnn_feature_columns, num_tasks, tasks, num_experts=4, expert_dim=8, dnn_hidden_units=(128, 128),
    ...
    dnn_out = Dense(units=128,activation='relu',kernel_initializer=glorot_normal(seed=1024),bias_initializer=Zeros())(dnn_input)
    dnn_out = Dense(units=128,activation='relu',kernel_initializer=glorot_normal(seed=1024),bias_initializer=Zeros())(dnn_out)
    ...
    task_name=['marital_stat','income']
    for index,mmoe_out in enumerate(mmoe_outs):
        logit = tf.keras.layers.Dense(
            1, use_bias=False, activation=None,name=task_name[index])(mmoe_out)
        # output = PredictionLayer(task)(logit)
        output = logit
        task_outputs.append(output)
